...
import { DefenderRole, type DefenderState } from '@/defense/DefenderCombatSystem';
...
export class KingdomsHudRenderer {
  ...
  setVisible(v: boolean): void {
    for (const obj of this.allObjects) {
      if (obj === this.bannerText) continue;  // banner only shown by showWaveClear/showGameOver/showVictory
      (obj as unknown as Phaser.GameObjects.Components.Visible).setVisible(v);
    }
    if (!v) this.bannerText.setVisible(false);
  }
}
